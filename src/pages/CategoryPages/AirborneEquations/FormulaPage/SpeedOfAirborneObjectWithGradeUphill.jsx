...
const numericFieldMapping = {
  ...
  function SpeedOfAirborneObjectWithGradeUphill() {
    ...
    return (
      <div className={"container mb-5 center"}>
        <Formula
          formulaName={"Speed of Object That Goes Airborne (Takeoff Grade Uphill)"}
          ...
          onCalculate={() => {
            ...
            if (isLower) {
              calculatedResult = ((2.73 * distance * Math.sqrt(1 + grade ** 2)) / Math.sqrt((input)+ ((distance) * grade)));
            } else {
              calculatedResult = ((2.73 * distance * Math.sqrt(1 + grade ** 2)) / Math.sqrt(((-1)*(input))+ ((distance) * grade)));
            }
...
export default SpeedOfAirborneObjectWithGradeUphill;
