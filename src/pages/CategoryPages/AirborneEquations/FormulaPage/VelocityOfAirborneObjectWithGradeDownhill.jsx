...
const numericFieldMapping = {
  ...
  function VelocityOfAirborneObjectWithGradeDownhill() {
    ...
    return (
      <div className={"container mb-5 center"}>
        <Formula
          formulaName={"Velocity of Object That Goes Airborne (Takeoff Grade Downhill)"}
          ...
          onCalculate={() => {
            ...
            if (isLower) {
                calculatedResult = ((401 * distance * Math.sqrt(1 + grade ** 2)) / Math.sqrt((input)+ (((-1)*(distance)) * grade)));
            } else {
              calculatedResult = ((401 * distance * Math.sqrt(1 + grade ** 2)) / Math.sqrt(((-1)*(input))+ (((-1)*(distance)) * grade)));
            }
...
export default VelocityOfAirborneObjectWithGradeDownhill;