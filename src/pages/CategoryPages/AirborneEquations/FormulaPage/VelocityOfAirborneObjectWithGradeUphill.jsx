...
const numericFieldMapping = {
  ...
  function VelocityOfAirborneObjectWithGradeUphill() {
    ...
    return (
      <div className={"container mb-5 center"}>
        <Formula
          formulaName={"Velocity of Object That Goes Airborne (Takeoff Grade Uphill)"}
          ...
          numericFields={[
            ...
            <NumericField
              key="grade"
              description="Takeoff Grade of the Vehicle (Uphill)"
          ...
          onCalculate={() => {
            ...
            if (isLower) {
              calculatedResult = ((401 * distance * Math.sqrt(1 + grade ** 2)) / Math.sqrt((input)+ ((distance) * grade)));
            } else {
              calculatedResult = ((401 * distance * Math.sqrt(1 + grade ** 2)) / Math.sqrt(((-1)*(input))+ ((distance) * grade)));
            }
            setResult(`Speed is ${calculatedResult.toFixed(3)}fps`);
          }}
        />
        {result !== null && (
          <p>{result}</p>
...
export default VelocityOfAirborneObjectWithGradeUphill;